...
const SupportTicketScreen = () => {
  ...
  const { ticket, loading: ticketLoading, error } = supportTicketDetails;
  const isLoading = !supportTicketDetails || ticketLoading;
  const [reply, setReply] = useState("");
  const [status, setStatus] = useState("");
  const [assignee, setAssignee] = useState("");
  const supportTicketReply = useSelector((state) => state.supportTicketReply);
  const supportTicketStatus = useSelector((state) => state.supportTicketStatus);
  const supportTicketAssign = useSelector((state) => state.supportTicketAssign);
  const { loading: isReplying, error: replyError } = supportTicketReply;
  const { loading: isUpdatingStatus, error: statusError } = supportTicketStatus;
  const { loading: isAssigning, error: assignError } = supportTicketAssign;
  const actionError = replyError || statusError || assignError;
  const admins = (userList?.users || []).filter((user) => user.isAdmin);
  ...
  const replyHandler = async (event) => {
    ...
    const ticket = await dispatch(replyToTicket({ id, message: reply }));
    if (!ticket) return;
    dispatch(getTicketById(id));
    setReply("");
  ...
  const statusHandler = async () => {
    const ticket = await dispatch(updateTicketStatus({ id, status }));
    if (!ticket) return;
    dispatch(getTicketById(id));
  ...
  const assignmentHandler = async () => {
    const ticket = await dispatch(
      assignTicket({ id, assignedTo: assignee || null }),
    );
    if (!ticket) return;
    dispatch(getTicketById(id));
  ...
  if (error || actionError)
    return (
      <div className="mx-auto max-w-4xl px-4 pt-24">
        <Alert type="error">
          {error?.data?.message || error?.message || error || actionError}
        </Alert>
      </div>
    );
      ...
      <div className="mx-auto max-w-4xl px-4 pb-24 pt-10 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 rounded-xl border border-slate-200 p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <span
              className={`rounded-full px-3 py-1 text-sm font-semibold capitalize ${statusClass(ticket.status)}`}
            >
              ...
              <div>
                ...
                <div className="mt-1 flex gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      isAssigning || assignee === (ticket.assignedTo?._id || "")
                    }
                    className="rounded-md bg-slate-800 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"
                  >
                    Save
        ...
        <section className="mt-8">
          ...
          <div className="mt-4 space-y-4">
            ...
            {ticket.replies.map((item) => (
              <article
                key={item._id}
                className={`rounded-xl border p-5 ${item.sender?._id === ticket.user?._id ? "border-slate-200 bg-white" : "border-indigo-100 bg-indigo-50"}`}
              >
                <div className="flex justify-between gap-4 text-sm">
                  <span className="font-semibold text-slate-900">
                    {item.sender?.name || "Support"}
                    {item.sender?.isAdmin ? " · Support" : ""}
                  </span>
                  <time className="text-slate-500">
        ...
        <form
          ...
          className="mt-8 rounded-xl border border-slate-200 p-5"
        >
          <label
